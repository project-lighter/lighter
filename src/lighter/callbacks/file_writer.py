...
@writer_registry.register(name="video")
def write_video(path: Path, tensor: torch.Tensor, *, suffix: str = ".mp4", fps: int = 24) -> None:
    """Write a video tensor to disk using :func:`torchvision.io.write_video`."""

    path = path.with_suffix(suffix)

    # Calculate padding for height and width to be divisible by 2
    H, W = tensor.shape[-2:]
    pad_h = (2 - H % 2) % 2
    pad_w = (2 - W % 2) % 2

    # Pad the tensor with the minimum value
    tensor = F.pad(tensor, (0, pad_w, 0, pad_h), mode="constant", value=tensor.min())
...
class FileWriter(BaseWriter):
    ...
    def setup(self, trainer: Trainer, pl_module: System, stage: str) -> None:  # type: ignore[override]
    ...
    @staticmethod
    def _to_sequence(outputs: dict[str, Any], key: str) -> list[Any]:
