...
class MetricContainer:
    ...
    def __init__(
        self,
        metric: Callable,
        adapter: MetricAdapter = DefaultMetricAdapter(),
    ...
    def _clean_dims_attribue(self, dims: Sequence[str]) -> Sequence[MetricResultDims]:
        ...
        for dim in dims:
            ...
            if dim is None:
                raise ValueError(f"Invalid dimension {dim}, " f"must be one of {MetricResultDims.__members__}")
            result.append(dim)
    ...
    def _check_dims(self, result: torch.Tensor, dims: Sequence[MetricResultDims]):
        ...
        try:
            ...
            if result.size(class_dim_index) != len(self.class_names):
                raise ValueError(
                    f"Invalid number of classes in result, expected {len(self.class_names)} ({self.class_names}), got {result.size(class_dim_index)}"
                )
        except ValueError:
            pass
